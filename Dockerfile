...
# Create venv at /opt (outside /workspace to avoid volume shadowing)
...
# Clone Applio to /opt (outside /workspace to avoid volume shadowing)
...
# Create workspace directories for USER DATA ONLY (persists via RunPod volume)
# Application code is at /opt/applio (outside /workspace, never shadowed)
RUN mkdir -p \
    /workspace/data/raw \
...
# Create Applio subdirectories in /opt (for application use, not user data)
RUN mkdir -p \
    /opt/applio/assets/datasets \
    /opt/applio/logs \
    /opt/applio/weights
...
# Set working directory to Applio (now at /opt instead of /workspace)
WORKDIR /opt/applio
